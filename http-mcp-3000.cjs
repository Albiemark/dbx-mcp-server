...
try {
  fs.writeFileSync(logFile, `${new Date().toISOString()}: HTTP MCP Server starting on port ${port}\n`);
} catch (error) {
  // Ignore errors
}
...
// Also support root endpoint
app.post('/', (req, res) => {
  const request = req.body;
  log(`Received at root: ${JSON.stringify(request)}`);
  
  try {
    handleRequest(request, (response) => {
      log(`Sending from root: ${JSON.stringify(response)}`);
      res.json(response);
    });
  } catch (error) {
    log(`Error at root: ${error.message}`);
    res.json({
      jsonrpc: '2.0',
      error: {
        code: -32603,
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', serverName: 'dbx-mcp-server', version: '0.1.0' });
});
